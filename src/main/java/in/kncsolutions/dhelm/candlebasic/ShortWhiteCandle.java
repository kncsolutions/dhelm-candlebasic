 ...
 */
 ...
 /**
 *According to literature a short white candle is made up of white body, i.e closing
 ...
 *candle. It means that the length of the candle including the shadow must be lower than the
 *average candle length of certain previous candles. The goal of this class is to calculate if a candle
 *is short white candle or not.   
 */
 public class ShortWhiteCandle{
   ...
   private boolean isShortWhiteCandle;
   /**
  ...
  */
   public ShortWhiteCandle(List<Double> open,List<Double> high,List<Double> low,List<Double> close, int numBody , int numLen, double percentage) throws DataException{
     Open.addAll(open);
	 High.addAll(high);
	 Low.addAll(low);
	 Close.addAll(close);
   ...
   *
   */
   private void ShortWhiteCandle()throws DataException{
   ...
   /**
   *@return Returns true if the latest candle is short white candle; otherwise returns false. 
   */
   public boolean isShortWhiteCandle(){
     return isShortWhiteCandle;
   }
 }