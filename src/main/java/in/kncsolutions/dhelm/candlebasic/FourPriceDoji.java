 ...
 */
 ...
 /**
 *According to litarature a  Four Price Doji candle is a candle where opening, closing, high and low prices are equal.
* The goal of this class is to calculate if a candle
 *is   white spinning top candle or not.   
 */
 public class FourPriceDoji{
   ...
   private boolean isFourPriceDoji=false;
   /**
  ...
  */
   public FourPriceDoji(List<Double> open,List<Double> high,List<Double> low,List<Double> close)throws DataException{
     Open.addAll(open);
	 High.addAll(high);
	 Low.addAll(low);
	 Close.addAll(close);
   ...
   *
   */
   private void FourPriceDoji()throws DataException{
     if(Open.get(0)==Close.get(0)){
	   if((Open.get(0)==High.get(0))&& (Close.get(0)==Low.get(0))){
	     isFourPriceDoji=true;
	   }
	 }
   }
   /**
   *@return true if the latest candle is a Four Price Doji Candle
   */
   public boolean isFourPriceDoji(){
   return isFourPriceDoji;
   }
 }