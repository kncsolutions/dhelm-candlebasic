 ...
 */
 ...
 /**
 *According to litarature a   Doji candle is a candle where opening and closing prices are equal.
 *However the rest of the definition is not clear. It is generally assumed that a candle is a simple
 *Doji candle if it is not a four price doji, long legged doji, gravestone doji or dragonfly doji.
 */
 public class Doji{
  ...
  */
   public Doji(List<Double> open,List<Double> high,List<Double> low,List<Double> close,int numLen,double percentage)throws DataException{
     Open.addAll(open);
	 High.addAll(high);
	 Low.addAll(low);
	 Close.addAll(close);
   ...
   *
   */
   private void Doji()throws DataException{
     double avgCandleLength=CandleFacts.getAverageLength(High,Low,RefLength); 
     if(Open.get(0)==Close.get(0)){
	   if(!(new FourPriceDoji(Open,High,Low,Close)).isFourPriceDoji()
	       && !(new LongLeggedDoji(Open,High,Low,Close,RefLength,Percentage)).isLongLeggedDoji()
		     && !(new GraveStoneDoji(Open,High,Low,Close,RefLength,Percentage)).isGraveStoneDoji()
			   && !(new DragonFlyDoji(Open,High,Low,Close,RefLength,Percentage)).isDragonFlyDoji()){			   
   ...
   /**
   *@return true if the latest candle is a  Doji Candle
   */
   public boolean isDoji(){
   return isDoji;
   }
    /**
   *@return true if the latest candle is a  Long Doji Candle
   */
   public boolean isLongDoji(){
   return isLong;
   }
 }