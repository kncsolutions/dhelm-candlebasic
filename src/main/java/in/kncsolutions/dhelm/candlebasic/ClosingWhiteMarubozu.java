 ...
 */
  ...
  */
   public ClosingWhiteMarubozu(List<Double> open,List<Double> high,List<Double> low,List<Double> close) throws DataException{
     Open.addAll(open);
	 High.addAll(high);
	 Low.addAll(low);
	 Close.addAll(close);
   ...
   */
   private void ClosingWhiteMarubozu()throws DataException{
	   ...
	   if(Open.get(0)<Close.get(0)){         
          if((High.get(0)==Close.get(0))&&(Low.get(0)<Open.get(0)) && (Close.get(0)-Open.get(0))>(Open.get(0)-Low.get(0))){            
