 ...
 */
 ...
 /**
 *According to litarature a  Gravestone Doji candle is a candle where opening and closing prices are equal.
 *High price is equal to the opening/closing price and low price is dintinct. It appears as a long candle.
 * The goal of this class is to calculate if a candle
 *is   Gravestone Doji candle or not.   
 */
 public class GraveStoneDoji{
   ...
   private boolean isGraveStoneDoji=false;
  /**
  ...
  */
   public GraveStoneDoji(List<Double> open,List<Double> high,List<Double> low,List<Double> close,int numLen,double percentage)throws DataException{
     Open.addAll(open);
	 High.addAll(high);
	 Low.addAll(low);
	 Close.addAll(close);
   ...
   *
   */
   private void GraveStoneDoji()throws DataException{
   ...
   /**
   *@return true if the latest candle is a Gravestone Doji Candle
   */
