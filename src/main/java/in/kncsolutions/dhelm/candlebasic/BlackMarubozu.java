 ...
 */
 ...
 /**
 *According to literature a  Black marubozu candle is made of Black body, i.e closing
 *price is lower than the opening price.There are no upper and lower shadows.
 *It means the high and low prices are equal to opening and closing prices respectively.
 * It  appears as a long or short candle. The goal of this class is to calculate if a candle
 *is  Black marubozu candle or not.   
 */
  ...
  */
   ...
   private void BlackMarubozu()throws DataException{
	   ...
	   if(Open.get(0)>Close.get(0)){         
          if((High.get(0)==Open.get(0))&&(Low.get(0)==Close.get(0))){            
                isBlackMarubozu=true; 				  
          }
	   }
	}
   /**
   *@return Returns true if the latest candle is a Black Marubozu candle; otherwise returns false. 
   */
   public boolean isBlackMarubozu(){
     return isBlackMarubozu;
   }
   /**
   ...
   *@return Returns true if the latest candle is long Black Marubozu candle; if short candle returns false. 
   *@throws DataException if sufficient data not available.
   */
   public boolean isLongBlackMarubozu(int numLen,double percentage)throws DataException{
   double avgCandleLength=CandleFacts.getAverageLength(High,Low,RefLength);
   if(isBlackMarubozu && (High.get(0)-Low.get(0))>avgCandleLength*Percentage/100)
     return true;
    return false;
   }
 }