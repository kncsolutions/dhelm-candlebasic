 ...
 */
 ...
 /**
 *According to litarature a Opening white marubozu candle is made of white body, i.e closing
 *price is higher than the opening price.There is an upper shadow but there is no lower shadow.
 *It means the high price is greater than close. The upper shadow should be smaller than the body.
 * It  appears as a long or short candle. The goal of this class is to calculate if a candle
 *is  Opening white marubozu candle or not.   
 */
  ...
  *@param close : List of close prices where first element is the latest data. 
  *@throws DataException if sufficient data not available.
  */
   public OpeningWhiteMarubozu(List<Double> open,List<Double> high,List<Double> low,List<Double> close) throws DataException{
     Open.addAll(open);
	 High.addAll(high);
	 Low.addAll(low);
	 Close.addAll(close);
   ...
   */
   private void OpeningWhiteMarubozu()throws DataException{
	   ...
	   if(Open.get(0)<Close.get(0)){         
          if((High.get(0)>Close.get(0))&&(Low.get(0)==Open.get(0)) && (Close.get(0)-Open.get(0))>(High.get(0)-Close.get(0))){            
                isOpeningWhiteMarubozu=true; 				  
          }
	   }
	}
   /**
   *@return Returns true if the latest candle is a Opening white Marubozu candle; otherwise returns false. 
   */
   public boolean isOpeningWhiteMarubozu(){
     return isOpeningWhiteMarubozu;
   }
   /**
   ...
   *@return Returns true if the latest candle is long Opening white Marubozu candle; if short candle returns false. 
   *@throws DataException if sufficient data not available.
   */
   public boolean isLongOpeningWhiteMarubozu(int numLen,double percentage)throws DataException{
   double avgCandleLength=CandleFacts.getAverageLength(High,Low,RefLength);
   if(isOpeningWhiteMarubozu && (High.get(0)-Low.get(0))>avgCandleLength*Percentage/100)
     return true;
    return false;
   }
 }