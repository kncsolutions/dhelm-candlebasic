 ...
 */
 ...
 /**
 *According to literature a short Black candle is made up of black body, i.e closing
 ...
 *None of the shadows can be greater than the body. It  appears as a short
 *candle. It means that the length of the candle including the shadows must be lower than the
 *average candle length of certain previous candles. The goal of this class is to calculate if a candle
 *is short Black candle or not.   
 */
 public class ShortBlackCandle{
   ...
   private boolean isShortBlackCandle;
   /**
  ...
  */
   public ShortBlackCandle(List<Double> open,List<Double> high,List<Double> low,List<Double> close, int numBody , int numLen, double percentage) throws DataException{
     Open.addAll(open);
	 High.addAll(high);
	 Low.addAll(low);
	 Close.addAll(close);
   ...
   *
   */
   private void ShortBlackCandle()throws DataException{
   ...
   /**
   *@return Returns true if the latest candle is short Black candle; otherwise returns false. 
   */
   public boolean isShortBlackCandle(){
     return isShortBlackCandle;
   }
 }