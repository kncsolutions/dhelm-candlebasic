 ...
 */
 ...
 /**
 *According to litarature a  Long Legged Doji candle is a candle where opening and closing prices are equal.
 *But high and low prices are dintinct. It appears as a long candle.
 * The goal of this class is to calculate if a candle
 *is   Long Legged Doji candle or not.   
 */
  ...
  */
   public LongLeggedDoji(List<Double> open,List<Double> high,List<Double> low,List<Double> close,int numLen,double percentage)throws DataException{
     Open.addAll(open);
	 High.addAll(high);
	 Low.addAll(low);
	 Close.addAll(close);
   ...
   private void LongLeggedDoji()throws DataException{
     ...
     if(Open.get(0)==Close.get(0)){
	   if((Open.get(0)<High.get(0))&& (Close.get(0)>Low.get(0))
	     ...
	     isLongLeggedDoji=true;
	   }
	 }
   }
   /**
   *@return true if the latest candle is a Long legged Doji Candle
   */
